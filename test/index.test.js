...
describe('Pair session',()=>{
    ...
    test('Debe devolver 0',()=>{

        expect(sum('')).toBe(0);

    })

    test('Cuando el input es un número', () => {
        expect(sum('3')).toBe(3)
    })

    test('Cuando el input son dos numeros', () => {
        expect(sum('1,2')).toBe(3)
    })

    test('Cuando el input contiene cualquier cantidad de numeros', () => {
        expect(sum('1,2,3,4')).toBe(10)
    })

    test('Cuando el input contiene cualquier cantidad de numeros dos delimitadores', () => {
        expect(sum('1\n2\n3')).toBe(6)
    })

    test('cuando se usa un delimiter custom', () => {
        expect(sum('//.\n1.2.3.4')).toBe(10)
    })

     test('numero negativo no permitido', () => {
        expect(sum('1,-2,3')).toBe('Negativos no permitidos: -2')
    })
})